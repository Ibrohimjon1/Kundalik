...
public class Qarzdorlar_royh extends AppCompatActivity {

    public static Qarzdorlar_adapter adapter = null;
    ArrayAdapter<String> adapter_auto;
    ListView gridView;
    ArrayList<String> list_auto;
    public static ArrayList<Qarzdorlar_list> list;
...
//        Intent intent = getIntent();
        tanish = Qarzdorlar_sana_yangi.qarzdor_ismi;
        ...
        txt_kirim_nomi_tanish.setText(tanish + "ni barcha qarzlar ro'yhati!");
        txt_kirim_nomi_tanish.setVisibility(View.VISIBLE);
        liniya.setVisibility(View.VISIBLE);

        final Cursor cursor = AsosiyOyna.sqLiteHelper.getData("SELECT name FROM TANISHLAR");
        ...
        try {
            int son = getdata("SELECT * FROM QARZDORLAR WHERE tanish_ismi LIKE '" + tanish.replace("'", "''") + "' ORDER BY sana_solish DESC");
            if (son == 0) {
                Toast.makeText(getApplicationContext(), "Qarzdorlarda hech nima yo'q!", Toast.LENGTH_SHORT).show();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        gridView.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                AlertDialog.Builder dialog = new AlertDialog.Builder(Qarzdorlar_royh.this);
                ...
                dialog.setItems(items, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int item) {
                        ...
                        if (item == 0) {
                            //Update
                            Cursor cursor1 = AsosiyOyna.sqLiteHelper.getData("SELECT tanish_ismi,summa,sana ,izox FROM QARZDORLAR WHERE Id LIKE '" + id_ozg + "'");
...
//                dialog.setCancelable(false);
        ...
        gridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, final View view, int position, long id) {
                ...
                Intent intent = new Intent(Qarzdorlar_royh.this, Qarzdorlar_izox.class);
        ...
        btn_kirim_filter.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (a == 2) {
                    getdata("SELECT * FROM QARZDORLAR WHERE tanish_ismi LIKE '" + tanish.replace("'", "''") + "' ORDER BY sana_solish ASC");
                    btn_kirim_filter.setImageResource(images_btn[1]);
                    a = 1;
                } else if (a == 1) {
                    getdata("SELECT * FROM QARZDORLAR WHERE tanish_ismi LIKE '" + tanish.replace("'", "''") + "' ORDER BY sana_solish DESC");
    ...
    public void onBackPressed() {
        super.onBackPressed();
        Qarzdorlar_sana_yangi.qarzdor_ismi = "";
    }
    ...
    private void showDialogUpdate_(final String id, final String tanish_ismi, final String summa, final String sana, final String izox) {
        ...
        final Dialog dialog = new Dialog(Qarzdorlar_royh.this);
            ...
            public void onClick(View v) {
                try {
                    if (!edt_summa.getText().toString().equals("") && !auto_tanish.getText().toString().equals("")) {
                        ...
                        String sql = "UPDATE QARZDORLAR SET tanish_ismi = ?, summa = ?, haq_summa = ?, izox = ? WHERE id = ?";
                        ...
                        int so = getdata("SELECT * FROM QARZDORLAR WHERE tanish_ismi LIKE '" + tanish.replace("'", "''") + "' ORDER BY sana_solish DESC");
                        if (so == 0){
                            finish();
                        }
                        dialog.dismiss();
...
//        edt_summa.addTextChangedListener(new NumberTextWatcherForThousand(edt_summa, 2, null));
        int width = Qarzdorlar_royh.this.getResources().getDisplayMetrics().widthPixels;
        int height = (int) (Qarzdorlar_royh.this.getResources().getDisplayMetrics().heightPixels);
    ...
    private void showDialogDelete(final ActionMode mode, final int deleting) {
        final AlertDialog.Builder dialogDelete = new AlertDialog.Builder(Qarzdorlar_royh.this);
        ...
        dialogDelete.setPositiveButton("Ha", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                try {
                    ...
                    AsosiyOyna.sqLiteHelper.deleteData_Shot_nomi(deleting, "DELETE FROM QARZDORLAR WHERE Id = ?");
                ...
                int asas = getdata("SELECT * FROM QARZDORLAR WHERE tanish_ismi LIKE '" + tanish.replace("'", "''") + "' ORDER BY sana_solish DESC");
                if (asas == 0) {
                    finish();
                }
            }
            ...
            public void onClick(View v) {
                try {
                    if (!edt_summa.getText().toString().equals("") && !auto_tanish.getText().toString().equals("")) {
                        ...
                        String sql = "INSERT INTO QARZDORLAR VALUES (NULL, ?, ?, ?, ?, ?, ?)";
